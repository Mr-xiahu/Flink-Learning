...
public class SftpHelperImpl implements IFtpHelper {
    ...
    public void loginFtpServer(String host, String username, String password,
        ...
        try {
            ...
            if (this.session == null) {
                throw new RuntimeException("创建ftp连接this.session失败,无法通过sftp与服务器建立链接，请检查主机名和用户名是否正确.");
            }
    ...
    @Override
    public Boolean dirIsExist(String directoryPath) {
        boolean isDirExist = false;
        try {
            this.printWorkingDirectory();
            SftpATTRS sftpATTRS = this.channelSftp.lstat(directoryPath);
    ...
    public InputStream getRemoteFileInputStream(String filePath) {
        try {
            this.printWorkingDirectory();
            String parentDir = filePath.substring(0,
                    StringUtils.lastIndexOf(filePath, IOUtils.DIR_SEPARATOR));
            this.channelSftp.cd(parentDir);
            this.printWorkingDirectory();
            InputStream readInputStream = this.channelSftp.get(filePath);
            String message = String.format(
                    "打开FTP文件[%s]获取读取流时出错,请确认文件%s有读取权限", filePath,
                    filePath);
            if (null == readInputStream) {
                throw new RuntimeException();
            }
            return readInputStream;
        } catch (SftpException e) {
            String message = String.format(
                    "读取文件[%s] 时出错,请确认文件%s有权限读取, errorMessage:%s", filePath,
                    filePath, e.getMessage());
            LOG.error(message);
            throw new RuntimeException();
        }
