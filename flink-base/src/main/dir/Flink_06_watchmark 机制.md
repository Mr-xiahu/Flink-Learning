...
~~~
1.dataStream流里面都是事件.假设每个事件都是一个Word对象,其中有如上介绍三种属性
2.存在两个window，window1的时间窗口为: 18:00:01-18:05:00 ,window2的时间窗口为:18:05:01-18:06:00
3.值得注意的是，watermark是依赖于事件时间生成的，目前生成规则为：'eventTimeStamp - 30000'
4.第一个事件来了，EventTime => 18:01:30   Watermark => 18:01:00 ,该事件进入window1
5.第二个事件来了，EventTime => 18:02:00   Watermark => 18:01:30 ,该事件进入window1
6....
7.第九个事件来了，EventTime => 18:05:20   Watermark => 18:04:50 注意:该事件时间为18:05:20，虽然这个事件时间超过了window1的限制，但是因为watermark的原因（处理延迟/乱序数据），该事件依然可以加入windwo1这个窗口等待计算
8.第十个事件来了，EventTime => 18:05:49   Watermark => 18:05:19 注意:此时watermark超过了该window1的设置,所以直接触发窗口进行计算,如果后面的新来了事件：EventTime => 18:04:49   Watermark => 18:04:19 ，该数据被丢弃,不做处理
~~~
...
Flink号称可以处理乱序/延迟数据，其实就是对于延迟数据的一种等待，或者说是对时间window阈值的增加
